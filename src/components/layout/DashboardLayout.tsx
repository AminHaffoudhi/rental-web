...
function DashboardBreadcrumb({ labels }: { labels: Record<string, string> }) {
                      ...
                      className={({ isActive }) =>
                        cn(
                          ...
                          isActive
                            ...
                            : "border-l-2 border-transparent text-[var(--surface-inverse-muted)] hover:bg-canvas-card/5 hover:text-[var(--surface-inverse-fg)]"
                        )
                      }
                    >
                      {({ isActive }) => (
...
export function DashboardLayout() {
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-canvas">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex h-full w-60 shrink-0 flex-col bg-stone-inv transition-transform duration-200 lg:static lg:h-auto lg:translate-x-0",
          mobileNavOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        )}
      >
        <button
          type="button"
          className="absolute right-3 top-4 rounded-lg p-2 text-[var(--surface-inverse-muted)] hover:bg-canvas-card/5 hover:text-white lg:hidden"
          onClick={closeMobile}
          aria-label="Close sidebar"
        >
          <X size={18} />
        </button>
        <SidebarNav onNavigate={closeMobile} isOwner={isOwner} sections={sections} />
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <header className="relative z-30 flex h-16 shrink-0 items-center justify-between overflow-visible border-b border-stone-200 bg-canvas-card px-4 shadow-elevated sm:px-8 dark:border-stone-800">
          <div className="flex items-center gap-3">
            ...
            <DashboardBreadcrumb labels={breadcrumbs} />
          </div>
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <NotificationBell />
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-30 flex items-center justify-around border-t border-stone-200 bg-canvas-card px-1 py-2 safe-bottom dark:border-stone-800 lg:hidden">
